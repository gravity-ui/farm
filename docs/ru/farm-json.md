...
- *String[]*
- *Optional*
> Список имен защищенных переменных окружения, которые нельзя переопределять при создании инстанса.
>
> Если пользователь попытается создать инстанс с переменными окружения из этого списка, сборка будет отклонена с ошибкой.
>
> Пример:
> ```json
> {
