...
- *Number*
- *Optional*
> Время в мс, по истечении которого инстанс будет остановлен. По умолчанию 1 час. При включённом `instanceActivityTrackingEnabled` активность, зарегистрированная через `touchInstance`, перезапускает таймер. Это значение используется, если для конкретного инстанса не указан свой `stopTimeout`. Если установлено в `-1`, инстансы будут жить вечно (автоматическая остановка отключена).

### `instanceActivityTrackingEnabled`
- *Boolean*
- *Optional*
> Включает остановку по времени последней активности. По умолчанию `false`. Когда настройка выключена, `touchInstance` не обновляет время активности, а таймер считается только от запуска процесса. Входной прокси должен посылать сигнал активности только для подходящих запросов и исключать проверки состояния.
