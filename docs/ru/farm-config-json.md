...
- *Number*
- *Optional*
> Время в мс, по истечении которого инстанс будет остановлен. По умолчанию 1 час. Это значение используется, если для конкретного инстанса не указан свой `stopTimeout`. Если установлено в `-1`, инстансы будут жить вечно (автоматическая остановка отключена).
...
- *Number*
- *Optional*
> Максимальная длина хэша инстанса. Если задано, хэш обрезается до этой длины.
