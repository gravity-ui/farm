...
- *Number*
- *Optional*
> Время в мс, по истечении которого инстанс будет остановлен. По умолчанию 1 час. Пользовательская или тестовая активность, зарегистрированная через `touchInstance`, перезапускает этот таймер. Повторная активность записывается не чаще раза в минуту. Внутренние проверки состояния на него не влияют. Это значение используется, если для конкретного инстанса не указан свой `stopTimeout`. Если установлено в `-1`, инстансы будут жить вечно (автоматическая остановка отключена).
